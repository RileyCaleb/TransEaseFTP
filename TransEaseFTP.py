...
class FTPUserManager:
    """FTP用户管理"""
    def __init__(self, config: FTPConfigManager):
        self.config = config
    ...
    def _setup_users(self) -> None:
        """从配置初始化用户"""
        if self.config.get('users', 'anonymous'):
            root_path = self.config.get('general', 'root_path')
            self.authorizer.add_anonymous(root_path, perm='elradfmwM')

    def add_user(self, username: str, password: str, homedir: str, permissions: str) -> bool:
        """添加新FTP用户"""
        try:
            self.authorizer.add_user(username, password, homedir, perm=permissions)
            return True
        except Exception as e:
...
class FTPMainWindow(QMainWindow):
    ...
    def _select_root_dir(self) -> None:
        ...
        if path:
            ...
            self.server_root.setText(f"根目录: {self.config.get('general', 'root_path')}")
